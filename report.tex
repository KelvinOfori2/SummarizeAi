...
Reading every document in full is rarely practical. Academic papers, technical reports, and long-form articles routinely run to dozens of pages when the relevant content might fill two or three. Manual summarization is slow and inconsistent; extractive NLP methods address both problems without requiring training data or cloud services.

This project implements a full-stack \textbf{Automatic Text Summarization System} that applies four extractive NLP algorithms to user-supplied documents. Users can paste raw text or upload PDF, DOCX, or TXT files up to 10\,MB. The system returns a condensed summary composed entirely of sentences drawn verbatim from the source, along with word-count statistics, compression ratio, and processing time. Summaries can be exported as PDF, DOCX, or plain text.

The system uses classical, unsupervised NLP methods rather than large language models: TF-IDF, Latent Semantic Analysis (LSA), LexRank, and Luhn. Because no new text is generated, every sentence in the output appeared verbatim in the source document.
...
A useful alternative would let users choose their algorithm and output length, work offline, show which sentences were selected, and accept PDF and DOCX files directly.

% --------------------------------------------------------------------------
\section{Objectives}

\begin{enumerate}[noitemsep]
  \item Implement four extractive NLP summarization algorithms: TF-IDF, LSA, LexRank, and Luhn.
...
\subsection{Extractive Summarization Approach}

Extractive summarization scores each sentence in the source document and returns the top-\textit{k} sentences in their original order, where \textit{k} is determined by the user-specified compression ratio. Because no new text is generated, the approach cannot produce fabricated content and returns the same output for the same input every time.
...
All four implemented algorithms are \textit{extractive}: they select and reorder existing sentences rather than generating new text. This keeps the system fast, dependency-light, and free of hallucination risk, but it caps summary quality in ways worth stating explicitly.

\begin{enumerate}[noitemsep]
  \item \textbf{No semantic understanding.} Sentences are ranked by word-frequency statistics (TF-IDF, Luhn), latent-semantic vector geometry (LSA), or graph centrality (LexRank), not by a model that understands what the text means. There is no transformer- or embedding-based component anywhere in the pipeline, which is what most contemporary summarization systems (e.g., BART, T5, GPT-based models) rely on.
  \item \textbf{Coherence gaps.} Although sentence order is preserved during selection, the extracted sentences originate from different parts of the source document. Stitching them together can produce dangling pronouns and references (e.g., "He argued that...") whose antecedent was in a sentence that was not selected, and can create abrupt topic transitions that a human editor would smooth over.
  \item \textbf{No compression or paraphrasing.} Because whole sentences are the unit of selection, a long sentence with only a few salient words is included in full. The system cannot shorten a sentence to its essential clause the way an abstractive summarizer can, so the compression ratio reflects sentence count rather than genuine information density.
  \item \textbf{Fixed, user-set ratio.} The summary length is a percentage chosen by the user rather than a length the algorithm derives from how much salient content the document actually contains, so short, information-dense documents and long, repetitive ones are compressed by the same rule.
\end{enumerate}

These limitations are inherent to the extractive paradigm rather than implementation defects, and were an accepted trade-off given the project's scope: a comparative study of classical, unsupervised summarization techniques rather than a competitor to modern LLM-based abstractive summarization. Section~\ref{sec:recommendations} outlines how an abstractive mode could be added alongside the existing algorithms in future work.

% --------------------------------------------------------------------------
\section{Conclusion}

This project delivers a complete, deployable text summarization platform built with open-source tools and unsupervised NLP algorithms. The four algorithms (TF-IDF, LSA, LexRank, and Luhn) suit different document types, from Luhn's fast heuristic scoring on short focused text to LexRank's graph-based ranking on longer prose.

The contribution beyond a basic NLP script is the surrounding infrastructure: per-account summary history, multi-format file support, three export formats, a WebSocket notification layer, and an admin panel. The PDF cleaning pipeline addresses a problem most open-source summarization tools ignore: noisy structured documents where table rows, repeated headers, and annotation lines compete with actual sentences during scoring. Cleaning this noise before ranking improved output quality without adding any machine learning.
...
\begin{enumerate}[noitemsep]
  \item \textbf{Add ROUGE evaluation.} Integrating the ROUGE-N metric against reference summaries would enable quantitative comparison of the four algorithms beyond processing time.
  ...
  \item \textbf{Abstractive comparison mode.} A lightweight abstractive model (e.g., BART or T5 via the Hugging Face pipeline) could be offered as a fifth algorithm with an explicit disclaimer about potential hallucinations.
